...
class FFN(nn.Module):
    def __init__(self, channel, reduction=8, drop_out=0.1):
        super(FFN, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Sequential(
            nn.Linear(channel, channel // reduction, bias=False),
            nn.GELU(), nn.Dropout(drop_out),
...
class SA(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.2):
        ...
        self.ffn = FFN(channel=d_model, reduction=8)
...
class CA(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        self.mu_attention1 = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=True)
        self.drop1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)

        self.mu_attention2 = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=True)
        self.drop2 = nn.Dropout(dropout)
        self.norm2 = nn.LayerNorm(d_model)
        ...
        self.ffn1 = FFN(channel=d_model, reduction=8)  
        self.ffn2 = FFN(channel=d_model, reduction=8)
    ...
    def forward(self, src1, src2,
        ...
        src1_cross = self.mu_attention1(query=src1,
                                         key=src2,
                                         value=src2, attn_mask=src2_mask,
                                         key_padding_mask=src2_key_padding_mask)[0]

        src2_cross = self.mu_attention2(query=src2,
                                         key=src1,
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        not_mask = ~mask
        y_embed = not_mask.cumsum(1, dtype=torch.float32)  # accumulate according to y dirction，(1，1，1)--(1，2，3)
        x_embed = not_mask.cumsum(2, dtype=torch.float32)  # accumulate according to x direction，(1，1，1).T--(1，2，3).T
        if self.normalize:
            eps = 1e-6
        ...
        pos = torch.cat((pos_y, pos_x), dim=3).permute(0, 3, 1, 2)  # The third dimension value is double of num_pos_feats
        # return pos
...
class FusionNet(nn.Module):
    #def __init__(self, backbone_dim=2048, c_dim=512, num_c=40):
    def __init__(self, backbone_dim=2048, c_dim=512, num_c=45):
        super(FusionNet, self).__init__()

        self.c_dim = c_dim  # channel number after reducing the dimension
        self.backbone_dim = backbone_dim
        self.droprate = 0.3  # transformer's droprate
        self.nheads = 8
        ...
        self.mutual_attention_list = nn.ModuleList([CA(c_dim, self.nheads, self.dim_feedforward, self.droprate)
    ...
    def forward(self, img_feat1, img_feat2):
        # channel attention
        img_feat1 = self.reduce_channel1(img_feat1)  # con1x1 
        ...
        img_feat1 = img_feat1.flatten(2)  # b f c
        img_feat2 = img_feat2.flatten(2)  # b f c

        feat1 = self.pos_rgb(img_feat1)
        feat2 = self.pos_depth(img_feat2)

        for mutual_attention in self.mutual_attention_list:
            feat1 = self.sa1(feat1)
            feat2 = self.sa2(feat2)
            feat1, feat2 = mutual_attention(feat1, feat2)
        ...
        img_feat1 = self.avgpool(img_feat1).squeeze(2)  
...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
        ...
        if not before_softmax and consensus_type != 'avg':  # the limit of consensus function, only can use avg consensus
    ...
    def forward(self, input):
        ...
        base_out1, base_out2, base_out3 = self.fusmodel(base_out_1, base_out_2)  # The first parameter is query
