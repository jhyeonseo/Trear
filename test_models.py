...
parser.add_argument('test_list', type=str, default='./annotations/FPHA_test_list.txt')
parser.add_argument('weights', type=str, default='./saved_model_name_appearance_model_best.pth.tar')
parser.add_argument('--arch', type=str, default="resnet50")
parser.add_argument('--save_scores', type=str, default="./test_result")
...
parser.add_argument('--dropout', type=float, default=0.5)   # ren's 0.5
...
def eval_video(video_data, network):
    ...
    if flag:
        rst_fus = rst_fus.reshape((num_crop, args.test_segments, num_class)).mean(axis=0).reshape(
            (args.test_segments, 1, num_class))
        return rst_fus, rst_rgb, rst_dep, label.numpy()[0]  # result of fusion, rgb, depth feature
    else:
        return i, rst_rgb, rst_dep, label.numpy()[0]        # result of rgb, depth
...
def main():
    ...
    if flag:
        ...
        print('Fusion accuracy {:.02f}%'.format(max(cls_acc_fus) * 100))
        #print('Fusion accuracy {:.02f}%'.format(np.mean(cls_acc_fus) * 100))
    print("***************************************************************************************************")
    print('RGB accuracy {:.02f}%'.format(max(cls_acc_rgb) * 100))
    print('Depth accuracy {:.02f}%'.format(max(cls_acc_dep) * 100))
    #print('RGB accuracy {:.02f}%'.format(max(cls_acc_rgb)) * 100)
    #print('Depth accuracy {:.02f}%'.format(max(cls_acc_dep)) * 100)
    print("***************************************************************************************************")
    ...
    if args.save_scores is not None:
        ...
        np.savez(args.save_scores + '/rgb_result', scores=reorder_output, labels=reorder_label)
        ...
        np.savez(args.save_scores + '/depth_result', scores=reorder_output, labels=reorder_label)
        if flag:
            ...
            #np.savez(args.save_scores + '_fusion', scores=reorder_output_3, labels=reorder_label_3)
            np.savez(args.save_scores + '/fusion_result', scores=reorder_output, labels=reorder_label)
